...
> **Distribution-Pivot 2026-05-27:** Phasen **8a (macOS Codesigning)** und **8b (Windows Authenticode-Signing)** sind **deprioritisiert** zugunsten der Web-Variante mit Linux-Server-OS (siehe `tasks/phase-server-web.md` + `ROADMAP.md` §"Distribution-Pivot 2026-05-27"). 8e (Tag v1.3.0) hängt daran und bleibt damit offen. Re-Aufnahme nur wenn breitere Desktop-Distribution mit "Verified publisher"-UX gewünscht wird. **8c (AppImage zsync)** und **8d (Nightly E2E)** sind bereits geshipt; **7h (Renderer-Smoke-Tests)** ist auch geshipt (PR #20). Linux-Build-Pfad bleibt aktiv.

**Ziel (Original-Spec):** Signierte/notarisierte Bundles auf allen drei OS, Self-Update-Mechanik für Linux, kontinuierliche cross-OS-E2E-Validierung. Voraussetzung für breitere Distribution außerhalb des Inner-Circles.
...
- [ ] ~~**Phase 8e — Tag v1.3.0 + Release**~~ — **deprioritisiert 2026-05-27** (hängt an 8a/8b, die deprio sind). Web-Distribution shippt unter eigenem Versions-Pfad via `docker-compose.example.yml` Image-Tag. Original-Spec: (1 h, deps: 8a-8d). Version-Bump 1.2.1 → 1.3.0 in root/gui/tauri parallel (siehe v1.2.0-PR #16-Pattern). All Gates: CI matrix grün, Bundle pipeline grün, signierte Artefakte auf allen drei OS manuell verifiziert, 3-Tage-Nightly grün, je OS ein Smoke-Test im Review-Sektion dokumentiert. `git tag v1.3.0` + push triggert tauri-bundle.yml.
...
- ~~**M8**: rpc.ts per-launch nonce/token Caller-Auth~~ → **end-to-end shipped + Codex-hardened 2026-05-24** (`feature/m8-hardening-codex-followup`). Core (2026-05-21): Sidecar `index.ts:36,44,46` generiert 128-bit-CSPRNG-nonce + emittiert stderr-handshake; Supervisor `supervisor.rs` parsed + attached an jeden Request; `rpc.ts handle()` validiert mit `-32001`; 6 Tests in `rpc.test.ts > "M8: nonce verification"`. Codex-Review 2026-05-24 fand **kein concrete bug**, aber 4 hardening-items implementiert: (1) Supervisor injiziert `CLAUDE_OS_RPC_NONCE=auto` (override gegen attacker-controlled launch-env); (2) stderr newline-buffering symmetric zu stdout (verhindert split-handshake-miss); (3) `set_nonce` first-handshake-only + 32-hex-format-validate (verhindert replay/format-attack); (4) Comment-fixes + ADR-0024-Trust-Boundary-Anhang dokumentiert MCP-`invoke()`-Bypass als separate Trust-Layer. +5 Rust-Tests (`is_valid_nonce` x3, `set_nonce` first-only + format-reject).
...
- [ ] **Phase Web-7-4 — Login + Registration GUI + Profile-Drawer** (4-6 h, M) — blockiert durch 7-3
  ...
  - `gui/src/components/AuthGate.tsx`: `/login` + `/register` als public.
  - Profile-Drawer (Sidebar-Header): email + logout + change-password-modal (analog SecretAddModal — siehe ADR-0022/0023).
  - **DoD:** End-to-end Browser-Flow register → login → use → logout → re-login. +8 GUI-Tests.
